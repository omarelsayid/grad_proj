...
class _State extends ConsumerState<HrTurnoverScreen> {
    ...
    return empsAsync.when(
      ...
      data: (allEmps) => rolesAsync.when(
        ...
        data: (roles) {
          ...
          final allRiskData = employees.map((emp) {
          ...
          return SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              ...filtered.map((r) => _RiskCard(riskData: r)),
            ]),
          );
        },
      ),
  ...
  Widget _buildPieChart(BuildContext context, Map<RiskLevel, int> levelDist) {
    final colorMap = {RiskLevel.low: AppColors.riskLow, RiskLevel.medium: AppColors.riskMedium, RiskLevel.high: AppColors.riskHigh, RiskLevel.critical: AppColors.riskCritical};
  ...
  Widget _buildFilters(List<String> depts, List<String> riskLevels) {
    return Row(children: [
      Expanded(child: DropdownButtonFormField<String>(
        value: _deptFilter, isDense: true,
      ...
      Expanded(child: DropdownButtonFormField<String>(
        value: _riskFilter, isDense: true,
...
class _RiskCard extends StatelessWidget {
  ...
  const _RiskCard({required this.riskData});
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('${riskData.employee.currentRole} • ${riskData.employee.department}', style: const TextStyle(fontSize: 11, color: Colors.grey)),
            ])),
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(color: _color.withOpacity(0.12), borderRadius: BorderRadius.circular(12)),
                child: Text(riskData.riskLevelLabel, style: TextStyle(color: _color, fontWeight: FontWeight.bold, fontSize: 12)),
              ),
              Text('${riskData.riskScore.toStringAsFixed(0)} pts', style: const TextStyle(fontSize: 11, color: Colors.grey)),
            ]),
          ]),
          const SizedBox(height: 8),
          LinearProgressIndicator(value: (riskData.riskScore / 163).clamp(0.0, 1.0), backgroundColor: _color.withOpacity(0.12), valueColor: AlwaysStoppedAnimation<Color>(_color), minHeight: 4),
          const SizedBox(height: 8),
          Wrap(spacing: 6, runSpacing: 4, children: riskData.factorBreakdown
              .where((f) => f.triggered)
              .map((f) => Container(
                padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                decoration: BoxDecoration(color: _color.withOpacity(0.08), borderRadius: BorderRadius.circular(8)),
